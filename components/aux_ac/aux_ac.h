...
// params
const float Constants::AC_MIN_TEMPERATURE = 15.0;
const float Constants::AC_MAX_TEMPERATURE = 33.0;
...
class AirCon : public esphome::Component, public esphome::climate::Climate {
       ...
       void _setStateMachineState(acsm_state state = ACSM_IDLE){
        ...
        void _debugPrintPacket(packet_t * packet, uint8_t dbgLevel = ESPHOME_LOG_LEVEL_DEBUG, unsigned int line = 0){
            ...
            // формируем данные
                ...
                if(ESPHOME_LOG_LEVEL_VERBOSE == dbgLevel){ 
        ...
        void _fillSetCommand(bool clrPacket = false, packet_t * pack = nullptr, ac_state_t *cmd = nullptr){
            ...
            // если команда не указана, значит выходим, лучше проверить пораньше
            if (cmd == nullptr) return;

            // очищаем пакет, если это указано
            if (clrPacket) _clearPacket(pack);

            // заполняем его параметрами из _current_ac_state - Уже не нужно
        ...
        bool sq_controlSmallStatus(){
            ...
            // проверяем ответ (так должно быть быстрее)
            if(  _inPacket.header->packet_type == AC_PTYPE_INFO &&
                 _inPacket.header->body_length == 0x0F          &&
                 _inPacket.body[0] == 0x01                      &&
                 _inPacket.body[1] == AC_CMD_STATUS_SMALL       ){
        ...
        bool sq_controlBigStatus(){
            ...
            // проверяем ответ
            if ( _inPacket.header->packet_type == AC_PTYPE_INFO &&
                 (_inPacket.header->body_length == 0x18 || _inPacket.header->body_length == 0x19) && // канальник Royal Clima отвечает пакетом длиной 0x19
                 _inPacket.body[0] == 0x01 &&
                 _inPacket.body[1] == AC_CMD_STATUS_BIG  ){
                // если пакет подходит, значит можно переходить к следующему шагу
        ...
        bool sq_controlDoCommand(){
            ...
            // проверяем ответ
            if ( _inPacket.header->packet_type == AC_PTYPE_INFO &&
                 _inPacket.header->body_length == 0x04 &&
                 _inPacket.body[0] == 0x01 &&
                 _inPacket.body[1] == AC_CMD_SET_PARAMS &&
                 // байты 2 и 3 обычно равны CRC отправленного пакета с командой
                 _inPacket.body[2] == _sequence[_sequence_current_step-1].packet.crc->crc[0] &&
                 _inPacket.body[3] == _sequence[_sequence_current_step-1].packet.crc->crc[1] ){
                // если пакет подходит, значит можно переходить к следующему шагу
    ...
    public:
        ...
        void stateChanged(){
            ...
            //
            // Brokly:
            // У меня есть на этот режим, кондей реагирует только в выключеном состоянии. Причем пульт отсылает
        ...
        void control(const esphome::climate::ClimateCall &call) override {
            ...
            // Правильнее проинициализировать эти данные из текущих
            // далее при необходимости достаточно будет проверить только cmd
            memcpy(&cmd, &_current_ac_state, sizeof(cmd)); 

            // User requested mode change
            if (call.get_mode().has_value()) {
                ClimateMode mode = *call.get_mode();
                switch (mode) { 
                    // воинствующим идиотам - сюда попадаем только в момент смены режима пользователем
                    ...
                    case climate::CLIMATE_MODE_FAN_ONLY:
                        ...
                        // GK: в режиме FAN TURBO работает , так что отключать не нужно!
                        ...
                        //  if(cmd.fanSpeed == AC_FANSPEED_AUTO){
            ...
            if (call.get_target_temperature().has_value()) {
                // выставлять температуру в режиме FAN, AUTO и OFF не нужно (не пищим кондеем лишьний раз)
                if ( cmd.mode != AC_MODE_FAN &&
                     cmd.mode != AC_MODE_AUTO && 
                     cmd.power != AC_POWER_OFF  ){
        ...
        // как оказалось сюда обращаются каждый раз для получения от нас любого параметра
